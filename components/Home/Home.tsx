...
const Home = () => {
  ...
  const { setAllProducts, onSetFilters } = useProducts();
  ...
  const mapAllProducts = () => {
    const threeProducts = products.slice(0, 12);
  ...
  //   if (!language) setCookie("selectedLanguage", "EN");
  // }, []);
  ...
  return (
    <main className="container mx-auto px-[15px] lg:px-[30px]">
      ...
      <Link className="relative w-full rounded-md text-white h-[177px] sm:h-auto flex flex-col items-center justify-center"
            href={PROVIDER_SITE+"?lang="+locale}>
          <Image
            className="absolute w-full h-full z-0"
            src={back1}
            alt="back1"
          ...
          {/* <p className="relative z-10 font-light">{translate.promotionsText}</p> */}
               { <p className="relative z-10 pt-4 px-3">{t('workWithUsText')}</p> }
