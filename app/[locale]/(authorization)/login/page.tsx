...
const LogIn: NextPage = () => {
  ...
  const { onLogInUser, onSetRegistrationPage, onSetForgetPassword, onGetUser } =
    useUserActions();
  ...
  const logInAccount: SubmitHandler<IInputsLogin> = () => {
    ...
    try {
      onLogInUser(requestBody).then((response) => {
  ...
  const handleCountryChange = (value: string) => {
    console.log(value);
    const country = prefixes.prefixes.find((item: Country) => item.id === value);
    if (!country) return;
    console.log(country);
    setCountryId(country.id);
    setPrefixCode(country.code);
  };
  ...
  useEffect(() => {
    ...
    const savedCountryId = getCookie("countryId");

    if (savedUser) setSelectPhone(savedUser);
    if (savedPrefix) setPrefixCode(savedPrefix);
    if (savedCountryId) setCountryId(savedCountryId);

    setValue("phone_login_number", String(savedUser));
  }, [rememberMe]);

  useEffect(() => {
    const defaultCountry = prefixes.prefixes.find(
      (item: Country) => item.id === countryId
    );
      ...
      <form onSubmit={handleSubmit(logInAccount)} className="form-content">
        ...
        <div className="form-content_bottom">
          ...
          <Button
            ...
            className="registrationButton text-white rounded-md w-full h-[44px] py-[10px]"
          >
