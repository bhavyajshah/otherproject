...
export async function generateStaticParams() {
  try {
    ...
    if (!Array.isArray(product.results)) {
      throw new Error("Expected an array from getAllProducts results");
    }

    return product.results.map(({ article_number }: { article_number: string }) => ({ article_number: article_number }));
  } catch (error) {
    console.error(error);
  }
}
